    ...
    create(context) {
        ...
        function getTemplateLiteral(currentNode, textBeforeNode, textAfterNode) {
            ...
            if (isConcatenation(currentNode) && !hasNonStringLiteral(currentNode)) {
                ...
                const parentNodeText = sourceCode.getText(currentNode.parent);

                if (parentNodeText.startsWith(currentNodeText)) {
                    const leftNodeText = currentNodeText.slice(0, currentNode.right.range[0] - firstToken.range[0]);

                    return `${leftNodeText}\`${currentNode.right.value}'`;
                }

                return `${firstToken.value.slice(0, -1)}\`${currentNodeText.slice(firstToken.value.length)}`;
            }
